...
function SelectPermissionsModal({ isModalOpen, setIsModalOpen, getValues, onConfirm, id, confirmLoading }: IProps) {
  ...
  const onCheck: TreeProps['onCheck'] = useCallback((checkedKeysValue: any) => {
    setModules((prevModules) => {
      const item = prevModules.find(i => i.code === moduleCode);
      if (item) {
        return prevModules.map((e) => {
      ...
      return [...prevModules, { code: moduleCode, menuIds: checkedKeysValue as number[] }];
    });
  }, [moduleCode]);
  ...
  const queryPermissionList = useCallback(() => {
  ...
  }, [activeKey, isModalOpen, queryPermissionList]);
