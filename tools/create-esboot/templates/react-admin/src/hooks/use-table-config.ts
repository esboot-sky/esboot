import type { ActionType } from '@dz-web/antd-pro-components';
import type { QueryKey } from '@tanstack/react-query';
import type { IJavaBaseResponse } from '@/api/instance';
...
interface TableConfigOptions<TParams, TData> {
...
interface TableRequestReturn<TData> {
  data: TData[] | undefined;
  total: number | undefined;
  success: boolean;
}

function useTableConfig<TParams extends Record<string, unknown>, TData = Record<string, unknown>>({
  defaultParams,
  fetch,
  queryKey,
  ...
  // @ts-expect-error expected generic argument
