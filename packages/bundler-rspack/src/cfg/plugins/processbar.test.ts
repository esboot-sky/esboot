...
describe('rspack processbar plugin', () => {
  ...
  it('adds progress plugin and reports used time at completion', async () => {
    ...
    const rspackCfg = { plugins: [] as any[] };

    await addProcessbarPlugin({ config: { isCIBuild: false } } as any, rspackCfg as any);

    expect(ProgressPlugin).toHaveBeenCalled();
    const pluginInstance = rspackCfg.plugins[0] as any;
