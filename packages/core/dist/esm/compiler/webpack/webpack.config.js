...
var _require4 = require(path.join(helpersPath, './config')),
  ...
  ESBOOT_IS_BROWSER = _require4.ESBOOT_IS_BROWSER,
  isDevMode = _require4.isDevMode,
  publicPath = _require4.publicPath;
...
var tsChecker = Number(process.env.tsChecker) !== 0;
var smp = new SpeedMeasurePlugin();
var globalScssPathList = [path.join(srcPath, './styles/'), path.join(srcPath, './platforms/mobile/styles/'), path.join(srcPath, './platforms/pc/styles/')];
...
var getPlugins = function getPlugins() {
  return [].concat(_toConsumableArray(entryList.map(function (i) {
    ...
    content: "\n    <script src=\"".concat(ESBOOT_RELATIVE_STATIC_CONFIG_PATH, "?v=").concat(process.env.BUILD_VERSION || pkg.version, "\"></script>\n    ").concat(!ESBOOT_IS_BROWSER && isDevMode ? "<script>\n      window.brigeMockHost = \"http://".concat(ip, "\";\n      window.brigeMockPort = ").concat(process.env.BRIDGE_MOCK_PORT || 3000, ";\n      </script>") : '', "\n    ")
  }), new webpack.DefinePlugin({
    VERSION: JSON.stringify(pkg.version),
    ENV: JSON.stringify(process.env.NODE_ENV)
  }), new FriendlyErrorsWebpackPlugin(), new CopyPlugin({
    patterns: userConfig.copyFile
  }), isDevMode && new ReactRefreshPlugin(), isDevMode && tsChecker && new ForkTsCheckerWebpackPlugin({})]);
};
  ...
  return [{
    ...
    use: [{
      ...
      options: {
        ...
        plugins: [].concat(_toConsumableArray(isDevMode && useMFSU ? mfsu.getBabelPlugins() : []), [isDevMode && require.resolve('react-refresh/babel')]).filter(Boolean)
      }
    }, {
        ...
        case 10:
          ...
          console.log(entryList.map(function (item) {
            return _objectSpread(_objectSpread({}, item), {}, {
              url: "http://".concat(ip, ":").concat(port).concat(!ESBOOT_IS_BROWSER ? "/".concat(item.name, ".html") : '')
            });
          }));
          _context.next = 16;
          break;
