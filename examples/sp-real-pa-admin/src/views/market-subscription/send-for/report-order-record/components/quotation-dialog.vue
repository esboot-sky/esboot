      ...
      <div v-if="form.packageId">
        <div class="item-box meal">
          ...
          <div class="meal-item">
            ...
            <span v-show="packageInfo.specialStart && packageInfo.specialEnd">~</span
...
export default defineComponent({
  ...
  setup(props: Record<string, any>, { emit }: Record<string, any>) {
    ...
    function dialogClean() {
      ...
      console.log(form.enableStart, '<___form.enableStart')
    ...
    function selectPackage() {
      quoListClick({ type: 2, enableStatus: 1, status: 10 }).then((res) => {
    ...
    // const querySearchAsync = (queryString: string, cd: any) => {
    //   if (queryString === "") {
    //     userInfo.value.cusNo = "";
    //     userInfo.value.mobile = "";
    //     userInfo.value.nickname = "";
    //   }
    //   setTimeout(() => {
    //     searchCondition({ cusNo: queryString }).then((res: Record<string, any>) => {
    //       cd(
    //         (res.result || []).map((v: Record<string, any>) => ({
    //           ...v,
    //           value: v.cusNo,
    //         }))
    //       );
