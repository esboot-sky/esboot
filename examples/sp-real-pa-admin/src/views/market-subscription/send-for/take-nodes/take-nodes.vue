<template>
  ...
  <div class="page-container">
    <div class="tool operate-buttons">
      ...
      <el-button
        v-permission="'order:export:list'"
        type="info"
      ...
      >
      <!-- <el-button
        v-permission="'order:batch:review'"
        type="info"
        :loading="isExportLoading"
        @click="reviewBatch"
        ...
        <el-table-column align="center" :label="$t('package_cycle')" width="100px">
          <template #default="scope">
            <div>
              ...
              <span v-if="scope.row.count == 2">{{ $t('natural_month') }}</span>
    ...
    <importDialog ref="importDialogRef" @callback="getData" :title="t('batch_gift')" />
...
import reviewBatchDialog from './review-batch-dialog.vue'
import baseTable from '@/components/base-table/base-table'
import baseFilter from '@/components/base-filter/base-filter'
import { orderListReq, orderDetail } from '@/api/quotation/order'
import { isGiveSelect, subTypeSelect, statusSelect, enableStatusSelect } from '@/utils/enum'
const infoRef = ref()
...
const tableConfig = reactive({
  ...
  customColumns: [
    ...
    { label: t('transaction_account'), prop: 'tradeAccount', width: '120', sortable: 'custom' },
    { label: t('fullName'), prop: 'username' },
    { label: t('nickname'), prop: 'nickname', width: '120' },
    { label: t('package'), prop: 'packageName', width: '140' },
    { slot: 'count' },
    { label: t('order_quantity'), prop: 'num', width: '100', sortable: 'custom' },
    { slot: 'price' },
    { label: t('expect_take_effect'), prop: 'enableStart', width: '120', sortable: 'custom' },
    { label: t('maturity_day'), prop: 'enableEnd', width: '120', sortable: 'custom' },
    { label: t('replenishment_time'), prop: 'createTime', width: '200', sortable: 'custom' },
    { slot: 'status' },
...
function selectPackage() {
  quoList({ type: 1, status: 10, pageNum: 1, pageSize: 10000 }).then((res) => {
...
function getData() {
  ...
  const page: Record<string, any> = { type: 1 }
  ...
  orderListReq(formToParams(params))
    .then((res) => {
...
const exportFunc = async () => {
  ...
  const actArr = reviewSelection.value.map((action: any) => action.id)
  console.log('actArr>>>', actArr)
  const params = { ...filterConfig.formProps.model, type: 1, ids: actArr }
  if (ordering.value.column) {
    Object.assign(params, {
      orders: [ordering.value],
    })
  }
  await exportData(params, `${t('order_history_streaming_quote')}.xls`)
